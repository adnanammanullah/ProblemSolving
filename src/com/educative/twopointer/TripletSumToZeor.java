package com.educative.twopointer;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashSet;
import java.util.List;
import java.util.Set;

public class TripletSumToZeor {
	
	public static List<List<Integer>> searchTriplets(int[] arr) {
		List<List<Integer>> output = new ArrayList<>();
		
		Arrays.sort(arr);
		List<Integer> pairList;
		for(int i=0; i<arr.length; i++) {
			
			if(i>0 && arr[i]==arr[i-1])
				continue;
			getPairWithSum(arr, i+1,-1*arr[i], output);
		
		}
		
		return output;
	}

	private static List<List<Integer>> getPairWithSum(int [] arr, int left, int targetSum, List<List<Integer>> pairList) {
		int j=arr.length-1;
		
		while(left<j) {
			if(arr[left]+arr[j]==targetSum) {
				pairList.add(Arrays.asList(-targetSum, arr[left], arr[j]));
				left++;
				j--;
			} else if(arr[left]+arr[j]>targetSum) {
				j--;
			} else
				left++;
		}
		return pairList;
	}

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		System.out.println(searchTriplets(new int[] { -3, 0, 1, 2, -1, 1, -2 }));
//		[[-3, 1, 2], [-2, 1, 1], [-2, 0, 2], [-1, 0, 1]]

	}
	

}
